...
class QuotaMdl {
    ...
    public function getList($quoteKind) {
        ...
        try {
            $this->qrystr = "SELECT c.id_cuota, ct.descri_cuota, c.campania, camp.obs AS mes, c.valor_1 AS valor_cuota_pura, c.fecha_venc_1, c.fecha_venc_2, c.fecha_venc_3, 
				...
				AND $where
				ORDER BY c.campania ASC, c.tipo_cuota";
...
//				require_once ($cfg['paso_helper'] . "/library_php.php");
				...
				// ********************************************************************************************************************************** //
				// busco todos los registros del transaction que esten sin procesar
				$qrystr_det = "SELECT 
												cu.*
				...
				foreach($filas_det AS $row_det) 
				{
					...
					if($id_cuota != 0)/// si encuentra una cuota para pagar la marco
					{
						...
						{ // si la cuota ya estaba pagada le agrego un error
							$qrystr_upd_cuota=" UPDATE paypertic_quote_transactions SET errors = CONCAT(NOW() $cfg[dif_hora],' // LA CUOTA $descripcion ($id_cuota) YA ESTABA PAGADA PREVIAMENTE EN EL MOMENTO $row_det[momento_pagada]') 
				...
				//Actualizar internal_status
				if($cant_procesados_error == 0)
					$internal_status = 2;
				else	
					$internal_status = 2;
				$qrystr_lote="UPDATE paypertic_transactions SET internal_status = 2'
											WHERE ppt_id_reg = '$pptId'
											";
				$data_lote = $conn->query($qrystr_lote);
				...
				$canal_pago = $row_canal['canal_pago'];
				...
				//echo "tit: $regional";
				$qrystr_nodo = "SELECT nodo, CURDATE() AS fecha FROM canal_pago_banco_cupon WHERE id_canal = $canal_pago";
				//echo $qrystr_nodo."<BR>";
				...
				if($acumulador_monto <> 0)
				{
					...
					$acumulador_interes = $acumulador_monto - $acumulador_cuota_pura;
